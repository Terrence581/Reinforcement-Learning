...
def type1_one_param(a1_origin, a2_origin, b1_origin, b2_origin, c_origin, target_origin):
    ...
    - L: an array for the first crossing time  
    """
    a1 = a1_origin.copy().reshape(-1); a2 = a2_origin.copy().reshape(-1)
    b1 = b1_origin.copy().reshape(-1); b2 = b2_origin.copy().reshape(-1)
    c = c_origin.copy().reshape(-1); target = target_origin.copy().reshape(-1)
    L = np.zeros(np.size(a1))
    #print(a1, b1, c)
    ...
    if case2.any():
        ...
        if case21.any():
            #print(a1, b1, c)
            L[case21] = np.int(np.log((target[case21] - c[case21])/a1[case21])/np.log(b1[case21])) + 1
        if case22.any():
            L[case22] = np.inf
...
def first_crossing_time_type1(a1_origin, a2_origin, b1_origin, b2_origin, c_origin, target_origin):
    ...
    - L: an array for the first crossing time  
    """
    if type(a1_origin) == np.float64:
        a1_origin = np.array([a1_origin]); b1_origin = np.array([b1_origin])
        a2_origin = np.array([a2_origin]); b2_origin = np.array([b2_origin])
        c_origin = np.array([c_origin]); target_origin = np.array([target_origin])
    a1 = np.copy(a1_origin); a2 = np.copy(a2_origin)
    b1 = np.copy(b1_origin); b2 = np.copy(b2_origin)
    c = c_origin.copy(); target = target_origin.copy()
    L = np.zeros(np.size(c))
